...
def Speak(data):
    with open("Interface/History/History.json", "r") as f:
        history = json.load(f)
    history[str(Soul)]["history"].append({"Data": data, "Date": time.strftime(
        "%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "Role": "bot"})
    ...
    if varient == "default":
        ...
        with open("Interface/History/History.json", "w") as f:
            history[str(soul)]["history"].append(
                {"Data": data, "Date": str(date), "Role": "user"})
            history[str(soul)]["history"].append(
                {"Data": "skeleton4jaris", "Date": str(date), "Role": "skeleton4jaris"})
            json.dump(history, f, indent=2)
...
def AddToUserHistoryImage(data, date, soul, role, img1, img2, img3, img4, varient="default"):
    ...
    if varient == "default":
        with open("Interface/History/History.json", "w") as f:
            history[str(soul)]["history"].append(
                {"Data": data, "Date": date, "Role": role, "Image": [img1, img2, img3, img4]})
    elif varient == "skeleton":
        with open("Interface/History/History.json", "w") as f:
            history[str(soul)]["history"].append(
                {"Data": data, "Date": date, "Role": role, "Image": []})
        json.dump(history, f, indent=2)
...
def CopyToClipboardForJS(data):
    ...
    toastNotification("Jarvis", "Copied to Clipboard",
                      f"Copied '{data}'", "short", f"{os.getcwd()}/Assets/Images/Jarvis.png", False)
...
def Return_Output(code, soul):
    ...
    with open("Interface/History/History.json", "r") as f:
        history = json.load(f)
    history[str(soul)]["history"].append({"Data": single_string, "Date": time.strftime(
        "%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "Role": "bot"})
    with open("Interface/History/History.json", "w") as f:
        json.dump(history, f, indent=2)
...
toSayWhenRecievedFile = ["Received a file, Sir.", "I have received a file, Sir.", "Opening a file that I received, Sir.", "A new file has arrived, Sir.", "I've successfully obtained a file, Sir.",
                         "File received, proceeding to open it, Sir.", "The file has been acquired, Sir.", "A file has been added to our system, Sir.", "A document has been delivered, Sir.", ]


def InititaliseFirebase(cred_json_filepath, db_url, storage_bucket):
    cred = credentials.Certificate(cred_json_filepath)
    firebase_admin.initialize_app(
        cred, {'storageBucket': storage_bucket, 'databaseURL': db_url})
...
def DownloadImage(image_url, filename="DownloadedImage.jpg"):
    ...
    if response.status_code == 200:
        ...
        print(
            f"#LOG: Failed to download the image. Status code: {response.status_code}")
...
def DeleteImagesFromFirebase(directory):
    ...
    for blob in blobs:
        try:
            ...
            print(
                f"Failed to delete image from Firebase Storage at: {blob.name}. Error: {e}")
...
def ImageRecieveAndToDoList(exit_flag, db_url=DB_URL, cred_json_filepath=Cred_JSON_Filepath):
    ...
    while not exit_flag.value:
        ...
        if current_value != '':
            ...
            file_save_path = DownloadImage(
                current_value, filename=f"{os.getcwd()}/Download/{time.time()}.jpg")
            os.startfile(file_save_path)

            try:
                DeleteImagesFromFirebase("images/")
            except IndexError:
                print("#LOG: Failed to extract the correct file path from the URL.")
        ...
        for task in tasks:
            if task.split(' ')[0] == IM_getCurrentTime():
                toastNotification("Jarvis To Do", "Task Time", task.split(
            ...
            while not exit_flag.value:
                ...
                if keyword_index >= 0:
                    ...
                    if foundQuery == False:
                        ...
                        # Bug Fixing
                        while responseGenCountCompletated < responseGenCount:
                            print("API beigh used: ",
                                  gemini_api_list[currentKeyIndex])
                            try:
                                with open(f'{os.getcwd()}\\Database\\Model\\Data\\history.txt', 'r') as f:
                                    res = Response(
                                        f.read(), Query, API=gemini_api_list[currentKeyIndex])
                                responseGenCountCompletated = 3
                                print(res)
                                currentKeyIndex += 1
                                if currentKeyIndex == numOfGeminiKeys:
                                ...
                                print(
                                    f"Error in Response function(Response.py), Error: {e}")
                                currentKeyIndex += 1
                    ...
                    if "EnterCache()" in res:
                        lines = res.split('\n')
                        filtered_lines = [
                            line for line in lines if "EnterCache()" not in line]
                        res = '\n'.join(filtered_lines)
                        UploadCache("1", {
                            "input": Query,
                            "output": f"{str(res)}"
...
def funcGUIprocess():
    toast = Notification(app_id="Jarvis", title="Jarvis is Up and Ready.", msg="Sir, your personal assistant Jarvis is up and is willing to do anything you want.",
                         duration="short", icon=f"{os.getcwd()}/Assets/Images/Jarvis.png")
    toast.show()

    VoiceExeProcess = multiprocessing.Process(
        target=funcVoiceExeProcess, args=(Exit,))
    VoiceExeProcess.start()

    ImageRecieveAndToDoListUpdateProcess = multiprocessing.Process(
        target=ImageRecieveAndToDoList, args=(Exit,))
    ImageRecieveAndToDoListUpdateProcess.start()

    try:
        eel.start("loading.html", position=(0, 0), close_callback=close,
                  block=True, size=(1500, 1200), port=8080)
